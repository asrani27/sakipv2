...
<table cellpadding="2" cellspacing="0" class="auto-style4" style="width: 100%">
    ...
    @foreach ($pk as $key => $item)
        ...
        <tr>
			...
			@if($rowspanIndikator)
			<td class="auto-style3" rowspan="{{count($item->indikator4)}}" align=center valign="top">{{$key+1}}</td>
            <td class="auto-style3" rowspan="{{count($item->indikator4)}}" valign="top">{{$item->kinerja_utama}}</td>
...
<table style="width: 100%">
	<tr>
		...
		<br class="auto-style7" />
		<br class="auto-style7" />
		<span class="auto-style7">{{$pegawai->nama}}</span><br class="auto-style7" />
		<span class="auto-style7">NIP. {{$pegawai->nip}}</span></td>
	</tr>
</table>
