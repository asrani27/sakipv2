...
<table border=1 cellspacing="0" cellpadding="2" style="width: 100%">
	...
	@foreach ($iku as $key =>$item)
		...
		<tr>
			...
			@if($rowspanIndikator)
			<td rowspan="{{count($item->indikator3)}}" class="auto-style10" align=center valign="top">{{$key + 1}}</td>
			<td rowspan="{{count($item->indikator3)}}" class="auto-style10" valign="top">{{$item->kinerja_utama}}</td>
			@php
